...
# 本分支（custom 版）仓库；上游原版见下方致谢与介绍文案
...
class AboutDialog(OverlayDialog):
    ...
    def __init__(self, parent=None):
        ...
        # 测试模拟包（MIWU_MOCK_DEVICES）在关于页明确标注
        _svc = getattr(parent, "_service", None)
        self._is_mock = bool(getattr(_svc, "is_mock", False))
        ...
        # ---- 标题 + 版本 ----
        self._title_label = QLabel("米家 - MiHome for Windows（custom 版）")
        ...
        build_tag = "custom 版"
        if self._is_mock:
            build_tag += " · 测试模拟包（MiHome-Windows-custom-mock）"
        self._version_label = QLabel(f"版本 v{__version__} · {build_tag}")
        self._version_label.setAlignment(Qt.AlignmentFlag.AlignHCenter)
        lay.addWidget(self._version_label)

        # ---- custom 版说明 ----
        self._fork_label = QLabel("custom 版")
        ...
        self._fork_desc_label = QLabel(
            f'本 custom 版基于上游原版 <a href="{UPSTREAM_URL}">huanyuejue/'
            f'MiHome-Windows</a> '
        ...
        self._github_btn = QPushButton(" 本 custom 版仓库 (iop666/MiHome-Windows)")
