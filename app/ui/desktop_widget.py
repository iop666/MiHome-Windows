...
class DesktopWidget(QWidget):
    ...
    def _build_device_block(self, did: str, dev):
        ...
        head.setSpacing(8)
        # 名称 + 房间分两行：名称在前（主页面改名后实时同步），过长自动换行
        text_col = QVBoxLayout()
        text_col.setContentsMargins(0, 0, 0, 0)
        text_col.setSpacing(2)
        name_lab = QLabel(name)
        name_lab.setWordWrap(True)
        name_lab.setAlignment(Qt.AlignmentFlag.AlignLeft
        ...
        if room:
            room_lab = QLabel(room)
            room_lab.setWordWrap(True)
            room_lab.setAlignment(Qt.AlignmentFlag.AlignLeft)
            room_lab.setFont(QFont("Microsoft YaHei UI", 8))
            room_lab.setStyleSheet(
                f"color: {self._col.TEXT_MUTED if online else self._col.OFFLINE_SUB};"
                " background: transparent;")
            text_col.addWidget(room_lab)
        text_host = QWidget()
        text_host.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground, True)
        text_host.setLayout(text_col)
        head.addWidget(text_host, 1)
