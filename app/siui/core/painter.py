...
from PySide6.QtGui import QColor, QLinearGradient, QPaintDevice, QPainter, QPainterPath


@contextmanager
def createPainter(device: QPaintDevice, render_hints: QPainter.RenderHints = QPainter.Antialiasing) -> QPainter:
...
def getRoundedRectPathQuad(
    rect: QRectF, radius_tl: float, radius_tr: float, radius_br: float, radius_bl: float
) -> QPainterPath:
    """
    使用 quadTo 绘制不对称圆角矩形
...
def getRoundedRectPathArc(
    ...
    path.lineTo(rect.right(), rect.bottom() - radius_br)
    path.arcTo(
        QRectF(rect.right() - 2 * radius_br, rect.bottom() - 2 * radius_br, 2 * radius_br, 2 * radius_br), 0, -90
    )
...
def getGaussianLinearGradient(
    start: QPointF | QPoint, final_stop: QRectF | QPoint, color: QColor, quality: int = 8
) -> QLinearGradient:
    start_x = start.x()
    start_y = start.y()
...
__all__ = ["createPainter", "getSuperRoundedRectPath", "getGaussianLinearGradient"]
